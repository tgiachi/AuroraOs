...
namespace AuroraOs.Common.Core.Utils
{
    ...
    public class AssemblyUtils
    {
        ...
        {
            return ScanAssembly(attribute, "*.dll");
        }
            ...
            if (_assembliesCache.Count == 0)
            {
                ...
                if (path == null)
                {
                    ...
                    foreach (var file in files)
                    {
                        try
                        {
                            var existsAssembly = (from s in allAssemblies where s.Location.Contains(file) select s).FirstOrDefault();
        ...
        public static void AddAssemblyToCache(string filename, Assembly assembly)
        {
            if (!_assembliesCache.Contains(assembly))
                _assembliesCache.Add(assembly);
        }
